...
from E_util.alphabet import Alphabet
from E_util.functions import *
...
class Config:
    ...
    def show_data_summary(self):
        ...
        print("\tNum corpus: %s" % (self.num_corpus))
    ...
    def generate_instance(self, input_file, name):
        self.fix_alphabet()
        # print('label---------')
        # for i in range(self.label_alphabet.size()):
        #     print(self.label_alphabet.get_instance(i))
        # print('---------label')
        if name == 'train':
            train_texts, train_ids = read_instance(input_file, self.word_alphabet, self.char_alphabet, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
            self.train_texts.append(train_texts)
            self.train_ids.append(train_ids)
        elif name == 'dev':
            dev_texts, dev_ids = read_instance(input_file, self.word_alphabet, self.char_alphabet, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
            self.dev_texts.append(dev_texts)
            self.dev_ids.append(dev_ids)
        elif name == 'test':
            test_texts, test_ids = read_instance(input_file, self.word_alphabet, self.char_alphabet, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
            self.test_texts.append(test_texts)
            self.test_ids.append(test_ids)
        elif name == 'raw':
            raw_texts, raw_ids = read_instance(input_file, self.word_alphabet, self.char_alphabet, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
            self.raw_texts.append(raw_texts)
            self.raw_ids.append(raw_ids)
        else:
